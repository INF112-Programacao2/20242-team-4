...
void janela_editar_dieta::setNutriId(int id) {
    this->_nutri_id = id;
}
...
void janela_editar_dieta::on__btn_salvar_clicked()
{
    ...
    QString dataFormatada = dataAtual.toString("dd/MM/yyyy HH:mm");

    // Pega o comentário atual e adiciona a nova edição
    QString comentarioAtual = ui->_te_comentario->toPlainText();
    QString novoComentario = comentarioAtual;

    // Adiciona uma linha em branco se o comentário atual não estiver vazio

    // Adiciona o registro da edição
    novoComentario += "   Editado por " + nomeNutricionista + " em " + dataFormatada;
    ...
    try {
        ...
        query_dieta.addBindValue(novoComentario);
        query_dieta.addBindValue(_user_id);
