...
void janela_login::playAudio(const QString& audioPath) {
    QUrl audioUrl(audioPath);
    if (!audioUrl.isValid()) {
        qDebug() << "URL do áudio inválida:" << audioPath;
        return;
    }

    player->stop(); // Para qualquer áudio que esteja tocando
    player->setSource(audioUrl);

    // Conecta um handler para verificar erros
    connect(player, &QMediaPlayer::errorOccurred, this, [this](QMediaPlayer::Error error) {
        qDebug() << "Erro ao reproduzir áudio:" << error;
    });
    ...
    , audioOutput(new QAudioOutput(this))
{
    ...
    player->setAudioOutput(audioOutput);  // Associa o QAudioOutput ao player
...
{
    playAudio("qrc:/audio/Audios/Cadastrar.mp3");
...
void janela_login::on__btn_entrar_clicked()
{
    ...
    playAudio("qrc:/audio/Audios/Entrar.mp3");
