...
#include "ui_janela_menu_nutricionista.h"

void janela_menu_nutricionista::setUserId(int id) {
    this->_user_id = id;
    {
        if (!conexao->abrir()) {
...
janela_menu_nutricionista::janela_menu_nutricionista(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::janela_menu_nutricionista)
{
    ui->setupUi(this);
...
janela_menu_nutricionista::~janela_menu_nutricionista()
{
    delete ui;
}

void janela_menu_nutricionista::on__btn_editar_clicked()
{
    ...
    editar = new janela_editar_dieta();
    editar->setUserId(id_cliente.toInt());
    editar->setNutriId(_user_id);
...
void janela_menu_nutricionista::on__btn_excluir_clicked()
{
    // Verifica se há uma linha selecionada
    int linha = ui->_tw_clientes->currentRow();
    if(linha < 0) {
    ...
    if(resposta == QMessageBox::Yes) {
        ...
        query.prepare("UPDATE table_perfil_usuario SET user_idnutricionista = NULL WHERE user_id = ?");
...
void janela_menu_nutricionista::on__btn_pesquisar_clicked()
{
